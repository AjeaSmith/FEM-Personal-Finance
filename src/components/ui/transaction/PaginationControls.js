...
function PaginationControls({
	...
	const paginationRange = useMemo(() => {
		const delta = 1; // Controls the number of pages shown around the current page
		const range = [];
	...
	return (
		<div className="flex justify-between items-center mt-4">
			<button
				className="flex items-center gap-x-2 px-[15px] py-2 border border-grey-900 bg-white text-grey-900 rounded-md disabled:opacity-50"
			...
			{/* Full Pagination for Tablet and Desktop */}
				...
				{Array.from({ length: totalPages }, (_, index) => index + 1).map(
					(page) => (
						<button
							...
							className={`px-[15px] py-2 rounded-md border border-grey-900 ${
								page === currentPage
									? "bg-grey-900 text-white" // Active page style
									: "text-grey-900 bg-white"
							}`}
						>
							{page}
						</button>
